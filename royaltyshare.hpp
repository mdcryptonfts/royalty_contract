...
CONTRACT royaltyshare : public contract {
	public:
		...
		ACTION claimroyalty();
		ACTION initconfig();
		ACTION insertdao(const name& user, const name& dao_name, const uint64_t& parts);
		ACTION insertfarm(const name& user, const name& farm_name, const uint64_t& parts);
		ACTION insertwallet(const name& user, const name& wax_address, const uint64_t& parts,
	...
	private:
		...
		farms_table farms_t = farms_table(FARM_CONTRACT, FARM_CONTRACT.value);
